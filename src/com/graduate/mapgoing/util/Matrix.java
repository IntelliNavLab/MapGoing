...
public class Matrix {
	 ...
	 * 
	 * @param args
	 *            参数a是个浮点型（double）的二维数组，b是浮点数；
	 * @return 返回值是一个浮点型二维数组（矩阵a乘以数b的结果）
	 */
	public static double[][] multib(double[][] a, double b) {
		int hang = a.length;
		int lie = a[0].length;
		double[][] result = new double[hang][lie];
		for (int i = 0; i < hang; i++) {
	...
	public static double det(double[][] a) {// Guass 消去
		double k = 0;
		double arrangerResult = (double) 1.0;
		...
		for (int i = 0; i < a.length; i++) {
			for (int j = 0; j < a[0].length; j++) {
				if (i == j) {
					arrangerResult = arrangerResult * a[i][j];// 计算主对角线相乘的结果
				}
			}
		}
